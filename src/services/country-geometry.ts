 ...
 * Self-hosted builds set VITE_COUNTRY_OVERRIDES_URL=/data/country-boundary-overrides.geojson:
 * the vendor CDN sends no Access-Control-Allow-Origin, so any other origin is blocked, and the
 * same file already ships in public/data. */
const COUNTRY_OVERRIDES_URL =
  import.meta.env?.VITE_COUNTRY_OVERRIDES_URL || 'https://maps.worldmonitor.app/country-boundary-overrides.geojson';
